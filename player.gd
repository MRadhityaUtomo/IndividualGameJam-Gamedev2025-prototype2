...
var graze_mana_rate: float = 35.0  # How much mana/sec to add during grazing

var is_reloading: bool = false
var reload_timer: float = 0.0
var reload_duration: float = 1.0

var graze_cooldown: float = 0.0  # prevent constant grazing
var graze_timer := 0.0

const normal_mana_regen = 5.0
var mana: float = 100.0
var max_mana: float = 100.0
var mana_regen_rate: float = normal_mana_regen  # per second
...
func _on_sleight_triggered(card_names: Array):
	...
	if combo == ["spread 3", "spread 3", "spread 3"].map(func(n): return n.to_lower()):
		fire_super_spread_burst(1, 0.5, 6, preload("res://card_sleights/sleight_combo1.tres"))
	
	if combo == ["spread 5", "spread 5", "spread 5"].map(func(n): return n.to_lower()):
		fire_super_spread_burst(1.5, 0.2, 8, preload("res://card_sleights/super_spread_big.tres"))
...
func fire_spirit_bomb(recipe: SleightRecipe):
	...
	mana_regen_rate = 0
	await get_tree().create_timer(2.5).timeout  # maybe some delay before regen
...
func _on_graze_area_entered(area):
	if area.is_in_group("enemy_bullet"):
		grazing_bullets += 1
		#print("✨ Graze Start | Grazing bullets:", grazing_bullets)

func _on_graze_area_exited(area):
	if area.is_in_group("enemy_bullet"):
		grazing_bullets = max(0, grazing_bullets - 1)
		#print("💨 Graze End | Grazing bullets:", grazing_bullets)
